...
## Подготовка
Для корректной работы GUI необходим HTTP сервер (иначе браузер будет блокировать запросы к другим файлам в целях безопасности).
Самый простой способ - [установить python][python-dl] и запустить сервер из консоли из директории с файлом index.html
...
Чтобы приложение работало с фейковым http backend'ом, необходимо создать модуль, который содержит в себе зависимость `ngMockE2E`, модули приложения (в примере ниже это `myApp`) и объявить фейковый backend (см. метод `run`)
